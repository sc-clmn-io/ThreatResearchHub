  ...
  private async fetchRSSFeed(source: ThreatSource): Promise<any> {
    ...
    if (typeof cleanData === 'string') {
      ...
      cleanData = cleanData
        ...
        .replace(/([a-zA-Z-]+)=([^"'>\s]+)(?=\s|>)/g, ' $1="$2"') // Add quotes to unquoted attributes
    ...
    try {
      ...
      // Extract items from RSS structure with multiple fallback paths
      if (result.rss?.channel?.item) {
        return Array.isArray(result.rss.channel.item) ? result.rss.channel.item : [result.rss.channel.item];
      }
      if (result.rss?.channel?.[0]?.item) {
        return Array.isArray(result.rss.channel[0].item) ? result.rss.channel[0].item : [result.rss.channel[0].item];
      }
      ...
      console.warn(`[TI] XML parsing failed, trying fallback method:`, parseError);
      // Fallback: try to extract basic RSS items using regex
      const itemMatches = cleanData.match(/<item[^>]*>[\s\S]*?<\/item>/g) || [];
      return itemMatches.slice(0, 10); // Limit to prevent issues
    }
  }
