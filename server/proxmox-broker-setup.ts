...
export async function testProxmoxConnectivity(req: Request, res: Response) {
  try {
    const { proxmoxHost = '192.168.100.188', sshUser = 'root' } = req.body;
    ...
    if (sshSuccess) {
      ...
      res.json({
        ...
        troubleshooting: [
          ...
          'Use Tailscale IP instead of 192.168.100.188',
...
export async function getProxmoxBrokerStatus(req: Request, res: Response) {
  try {
    const { proxmoxHost = '192.168.100.188', sshUser = 'root' } = req.query;
...
export async function configureVMLogForwarding(req: Request, res: Response) {
  try {
    const { proxmoxHost = '192.168.100.188', vmId, sshUser = 'root' } = req.body;
    ...
    const logForwardingScript = `
      ...
      if [ "$VM_IP" != "VM_IP_NOT_FOUND" ]; then
        ...
        qm guest exec ${vmId} -- bash -c "
          echo '*.* @@192.168.100.188:514' >> /etc/rsyslog.d/99-xsiam-forward.conf
          systemctl restart rsyslog
    ...
    res.json({
      ...
      manualSteps: [
        `SSH into VM ${vmId}`,
        'Add to /etc/rsyslog.d/99-xsiam-forward.conf: *.* @@192.168.100.188:514',
