...
import { comprehensiveSanitizer } from './comprehensive-sanitizer.js';
...
function generate30DayThreatDatabase() {
  ...
  const threatTemplates = [
    ...
    },
    {
      ...
      description: 'CVE-YYYY-NNNN allows container escape through malicious ingress configurations in Kubernetes environments.',
      category: 'cloud',
      severity: 'critical',
      cves: ['CVE-YYYY-NNNN'],
...
// XSIAM Content Upload API endpoints
  ...
  // Add threat infrastructure mapping routes
  app.get('/api/threat-infrastructure/mappings', async (req, res) => {
    try {
      const mappings = threatInfrastructureMapper.getThreatMappings();
      res.json(mappings);
    } catch (error: any) {
  ...
  app.get('/api/threat-infrastructure/threats', async (req, res) => {
    try {
      const threats = threatInfrastructureMapper.getAvailableThreats();
      res.json(threats);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post('/api/threat-infrastructure/deploy', async (req, res) => {
    try {
      const { threatScenario, platform } = req.body;
      const mapping = await threatInfrastructureMapper.generateInfrastructureForThreat(threatScenario);
      
      if (!mapping) {
        return res.status(404).json({ error: 'Threat scenario not found' });
      }

      if (platform === 'proxmox') {
        const result = await threatInfrastructureMapper.deployProxmoxInfrastructure(mapping, threatScenario);
        res.json(result);
      } else {
        res.json({ mapping, message: 'Infrastructure mapping generated' });
      }
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  // Docker Host Management API endpoints
  app.get("/api/proxmox/docker-vms", async (req, res) => {
    try {
      console.log('Fetching real Docker VMs from Proxmox...');
      
      // Get list of VMs from Proxmox
      const listCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "qm list | grep -E '(running|stopped)'"`;
      
      let dockerVMs = [];
      
      try {
        const listResult = await execAsync(listCommand);
        const vmLines = listResult.stdout.trim().split('\n');
        ...
        for (const line of vmLines) {
          ...
          if (parts.length >= 3) {
            ...
            // Check if this is likely a Docker VM (by name or ID)
            if (name.includes('docker') || vmid === '201') {
              console.log(`Found Docker VM: ${name} (${vmid}) - ${status}`);
              
              // Get detailed VM info
              const configCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "qm config ${vmid}"`;
              let memory = "8192", cores = "4", vmIP = `192.168.1.${200 + parseInt(vmid) - 200}`;
              
              try {
                const configResult = await execAsync(configCommand);
                const config = configResult.stdout;
                
                // Parse memory and cores from config
                const memoryMatch = config.match(/memory:\s*(\d+)/);
                const coresMatch = config.match(/cores:\s*(\d+)/);
                
                if (memoryMatch) memory = memoryMatch[1];
                if (coresMatch) cores = coresMatch[1];
                
                // Try to get IP if VM is running
                if (status === 'running') {
                  try {
                    const ipCommand = `ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@100.64.0.1 "qm agent ${vmid} network-get-interfaces 2>/dev/null | grep 'ip-address' | head -1 | cut -d'\"' -f4"`;
                    const ipResult = await execAsync(ipCommand);
                    if (ipResult.stdout.trim()) {
                      vmIP = ipResult.stdout.trim();
                    }
                  } catch (e) {
                    console.log(`Could not get IP for VM ${vmid}`);
                  }
                }
              } catch (e) {
                console.log(`Could not get config for VM ${vmid}`);
              }
              
              // Check Docker status if VM is running
              let dockerVersion = null;
              let containers = [];
              
              if (status === 'running' && vmIP) {
                try {
                  const dockerCheckCommand = `ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@${vmIP} "docker --version 2>/dev/null"`;
                  const dockerResult = await execAsync(dockerCheckCommand);
                  if (dockerResult.stdout.includes('Docker version')) {
                    dockerVersion = dockerResult.stdout.split(' ')[2]?.replace(',', '');
                    
                    // Get running containers
                    const containersCommand = `ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@${vmIP} "docker ps --format 'table {{.ID}}\\t{{.Names}}\\t{{.Image}}\\t{{.Status}}\\t{{.Ports}}\\t{{.CreatedAt}}' --no-trunc"`;
                    const containersResult = await execAsync(containersCommand);
                    
                    if (containersResult.stdout) {
                      const containerLines = containersResult.stdout.split('\n').slice(1); // Skip header
                      containers = containerLines.filter(line => line.trim()).map(line => {
                        ...
                        return {
                          ...
                          created: parts[5] || new Date().toISOString()
                        };
                      });
                    }
                  }
                } catch (e) {
                  console.log(`Could not check Docker status for VM ${vmid}`);
                }
              }
              ...
              dockerVMs.push({
                ...
                memory,
                cores,
                disk: "50", // Default, could be parsed from config
                ip: vmIP,
                dockerVersion,
                containers
              });
        ...
        console.error('Error fetching VMs from Proxmox:', error);
        // Return empty array if can't connect to Proxmox
        dockerVMs = [];
      }
      ...
      res.json({
        success: true,
        vms: dockerVMs,
        count: dockerVMs.length,
        timestamp: new Date().toISOString()
      });
    } catch (error: any) {
      console.error('Docker VMs retrieval error:', error);
      res.status(500).json({ 
        success: false, 
        error: error.message || 'Failed to retrieve Docker VMs' 
      });
    }
  });

  app.post("/api/proxmox/create-docker-vm", async (req, res) => {
    try {
      const { vmid, name, memory, cores, disk, template } = req.body;
      
      if (!vmid || !name) {
        return res.status(400).json({ 
          success: false, 
          error: 'VM ID and name are required' 
        });
      }

      console.log(`Creating real Docker VM: ${name} (${vmid}) with ${memory}MB RAM, ${cores} cores`);

      // Step 1: Create VM with basic configuration
      const createCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "
        qm create ${vmid} --name ${name} --memory ${memory || 8192} --cores ${cores || 4} --net0 virtio,bridge=vmbr0 &&
        qm set ${vmid} --scsi0 local-lvm:${disk || 50},format=qcow2 &&
        qm set ${vmid} --boot c --bootdisk scsi0 &&
        qm set ${vmid} --ostype l26 &&
        qm set ${vmid} --scsihw virtio-scsi-pci
      "`;

      console.log('Executing VM creation...');
      const createResult = await execAsync(createCommand);
      
      // Step 2: Download and set Ubuntu Cloud-Init template
      const templateCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "
        cd /var/lib/vz/template/iso &&
        wget -O ubuntu-22.04-server.qcow2 https://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img &&
        qm importdisk ${vmid} ubuntu-22.04-server.qcow2 local-lvm --format qcow2 &&
        qm set ${vmid} --scsi0 local-lvm:vm-${vmid}-disk-0,cache=writeback,discard=on,ssd=1 &&
        qm set ${vmid} --ide2 local-lvm:cloudinit &&
        qm set ${vmid} --serial0 socket --vga serial0
      "`;

      console.log('Setting up Ubuntu template...');
      await execAsync(templateCommand);

      // Step 3: Configure Cloud-Init for Docker installation
      const cloudInitCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "
        qm set ${vmid} --ciuser root &&
        qm set ${vmid} --cipassword 'ThreatResearch2025!' &&
      ...
      await execAsync(cloudInitCommand);

      // Step 4: Start VM
      const startCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "qm start ${vmid}"`;
      
      console.log('Starting VM...');
      await execAsync(startCommand);

      // Step 5: Wait for VM to boot and install Docker
      console.log('Waiting for VM to boot...');
      await new Promise(resolve => setTimeout(resolve, 30000)); // Wait 30 seconds for boot

      // Get VM IP address
      const ipCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "qm agent ${vmid} network-get-interfaces | grep -A 3 'name.*eth0' | grep 'ip-address' | head -1 | cut -d'\"' -f4"`;
      let vmIP = '192.168.100.201'; // Default fallback
      ...
      try {
        ...
        console.log('Could not get VM IP, using default');
      }

      // Step 6: Install Docker on the VM
      const dockerInstallCommand = `ssh -o ConnectTimeout=30 -o StrictHostKeyChecking=no root@${vmIP} "
        apt update -y &&
      ...
      try {
        await execAsync(dockerInstallCommand);
      } catch (e) {
        console.log('Docker installation may need manual verification');
      }

      // Step 7: Configure log forwarding to XSIAM broker
      const logForwardingCommand = `ssh -o ConnectTimeout=30 -o StrictHostKeyChecking=no root@${vmIP} "
        echo '*.* @@192.168.100.124:514' >> /etc/rsyslog.conf &&
        systemctl restart rsyslog &&
        echo 'Log forwarding configured to XSIAM broker'
      "`;

      console.log('Configuring log forwarding...');
      try {
        await execAsync(logForwardingCommand);
      } catch (e) {
        console.log('Log forwarding configuration may need manual setup');
      }
      ...
      res.json({
        ...
        name,
        ip: vmIP,
        message: `Real Docker VM ${name} created successfully on Proxmox`,
        details: {
          vmid: vmid,
          name: name,
          memory: memory || 8192,
          ...
          logForwarding: 'configured'
        },
        nextSteps: [
          'Docker Engine installed and running',
          'Log forwarding configured to XSIAM broker (192.168.100.124:514)',
          'VM ready for threat scenario deployment',
          `Access via SSH: ssh root@${vmIP}`
        ]
      });
    } catch (error: any) {
      console.error('Real Docker VM creation error:', error);
      res.status(500).json({ 
        success: false, 
        error: error.message || 'Failed to create real Docker VM',
        details: 'Check Proxmox server connectivity and SSH access'
      });
    }
  });

  app.post("/api/proxmox/configure-docker-logs", async (req, res) => {
    try {
      const { vmid, brokerIP, brokerPort } = req.body;
      
      if (!vmid || !brokerIP) {
        return res.status(400).json({ 
          success: false, 
          error: 'VM ID and broker IP are required' 
        });
      }

      // Configure Docker daemon to forward logs to XSIAM broker
      const logConfig = {
        "log-driver": "syslog",
        "log-opts": {
          "syslog-address": `tcp://${brokerIP}:${brokerPort || '514'}`,
          "tag": "docker-{{.Name}}"
        }
      };

      console.log(`Configuring log forwarding for VM ${vmid} to ${brokerIP}:${brokerPort}`);
      ...
      res.json({
        ...
        vmid,
        brokerIP,
        brokerPort: brokerPort || '514',
        message: 'Log forwarding configured successfully',
        logDriver: 'syslog',
        logAddress: `tcp://${brokerIP}:${brokerPort || '514'}`
      });
    } catch (error: any) {
      console.error('Log forwarding configuration error:', error);
      res.status(500).json({ 
        success: false, 
        error: error.message || 'Failed to configure log forwarding' 
      });
    }
  });

  // Real Docker Scenario Deployment Endpoint
  app.post("/api/docker/deploy-scenario", async (req, res) => {
    try {
      const { vmid, scenario, containers } = req.body;
      
      if (!vmid || !scenario || !containers) {
        return res.status(400).json({ 
          success: false, 
          error: 'VM ID, scenario, and containers are required' 
        });
      }

      console.log(`Deploying real ${scenario} scenario to VM ${vmid}`);

      // Get VM IP first
      const ipCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@100.64.0.1 "qm agent ${vmid} network-get-interfaces 2>/dev/null | grep 'ip-address' | head -1 | cut -d'\"' -f4"`;
      let vmIP = '192.168.100.201'; // Default fallback
      ...
      try {
        ...
        console.log('Using default IP for VM');
      }

      console.log(`Deploying containers to VM at ${vmIP}`);

      const deployedContainers = [];
      
      // Deploy each container in the scenario
      for (const container of containers) {
        try {
          console.log(`Deploying container: ${container.name}`);
          
          // Create Docker run command based on container configuration
          let dockerCommand = `docker run -d --name ${container.name}`;
          ...
          // Add log forwarding configuration based on type
          switch (container.logForwarding.type) {
            case 'Application Log':
              dockerCommand += ` --log-driver=syslog --log-opt syslog-address=tcp://192.168.100.124:514 --log-opt tag=${container.name}`;
              break;
            case 'Syslog Forwarder':
              dockerCommand += ` --log-driver=syslog --log-opt syslog-address=udp://192.168.100.124:514 --log-opt tag=${container.name}`;
              break;
            case 'Custom Integration':
              dockerCommand += ` -e XSIAM_BROKER=192.168.100.124:514 -e LOG_TYPE=json`;
              break;
            case 'XSIAM Agent':
              dockerCommand += ` -v /var/log:/var/log:rw -e XSIAM_BROKER=192.168.100.124:514`;
              break;
          }
          
          // Add the image
          dockerCommand += ` ${container.image}`;
          
          // Execute the Docker command on the VM
          const fullCommand = `ssh -o ConnectTimeout=30 -o StrictHostKeyChecking=no root@${vmIP} "${dockerCommand}"`;
          
          console.log(`Executing: ${dockerCommand}`);
          const result = await execAsync(fullCommand);
          
          // Get container details
          const containerInfoCommand = `ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@${vmIP} "docker inspect ${container.name} --format='{{.Id}}|{{.State.Status}}|{{.Created}}'"`;
          const infoResult = await execAsync(containerInfoCommand);
          
          const [containerId, status, created] = infoResult.stdout.trim().split('|');
          ...
          deployedContainers.push({
            ...
            logForwarding: container.logForwarding,
            description: container.description
          });
          
          console.log(`✅ Container ${container.name} deployed successfully`);
          
        } catch (error) {
          console.error(`❌ Failed to deploy container ${container.name}:`, error);
          deployedContainers.push({
            name: container.name,
            id: '',
            image: container.image,
      ...
      // Configure log forwarding agents if needed
      console.log('Configuring log forwarding agents...');
      
      const logConfigCommand = `ssh -o ConnectTimeout=30 -o StrictHostKeyChecking=no root@${vmIP} "
        # Configure rsyslog for container logs
        echo '*.* @@192.168.100.124:514' >> /etc/rsyslog.conf
        systemctl restart rsyslog
        
        # Install and configure Filebeat for application logs
        curl -L -O https://artifacts.elastic.co/downloads/beats/filebeat/filebeat-8.8.0-amd64.deb
        dpkg -i filebeat-8.8.0-amd64.deb
        
        # Configure Filebeat to send to XSIAM broker
...
output.logstash:
  hosts: ['192.168.100.124:5044']

EOF
        systemctl enable filebeat
        systemctl start filebeat
        
        echo 'Log forwarding agents configured'
      "`;
      
      try {
        await execAsync(logConfigCommand);
        console.log('✅ Log forwarding agents configured');
      } catch (e) {
        console.log('⚠️ Log forwarding agents may need manual configuration');
      }
      ...
      res.json({
        ...
        vmIP,
        scenario,
        containersDeployed: deployedContainers.length,
        containers: deployedContainers,
        message: `${scenario} scenario deployed successfully to VM ${vmid}`,
        logForwarding: {
          broker: '192.168.100.124:514',
          methods: ['rsyslog', 'filebeat', 'docker-syslog'],
          status: 'configured'
        },
        accessInstructions: [
          `SSH to VM: ssh root@${vmIP}`,
          `View containers: docker ps`,
          `View logs: docker logs <container_name>`,
          `Test applications: Use exposed ports ${deployedContainers.flatMap(c => c.ports).join(', ')}`
        ]
      });
      
    } catch (error: any) {
      console.error('Docker scenario deployment error:', error);
      res.status(500).json({ 
        success: false, 
        error: error.message || 'Failed to deploy Docker scenario',
        details: 'Check VM connectivity and Docker daemon status'
      });
    }
  });

  // Comprehensive Repository Sanitization endpoint
  app.post("/api/sanitize/repository", async (req, res) => {
    try {
      console.log('[API] Starting comprehensive repository sanitization...');
      
      const result = await comprehensiveSanitizer.sanitizeEntireRepository('.');
      
      res.json({
        success: true,
        message: 'Repository sanitization completed',
        ...result,
        safeForPublicSharing: result.employmentRiskIssuesFound === 0
      });
    } catch (error: any) {
      console.error('Error in repository sanitization:', error);
      res.status(500).json({ 
        success: false, 
  ...
  // Git history sanitization endpoint
  app.post("/api/sanitize/git-history", async (req, res) => {
    try {
      await comprehensiveSanitizer.sanitizeGitHistory();
      
      res.json({
        success: true,
        message: 'Git history sanitization instructions provided',
        note: 'Manual git-filter-repo execution required'
      });
    } catch (error: any) {
      console.error('Error in git history sanitization:', error);
      res.status(500).json({ 
        success: false, 
        error: 'Git history sanitization failed', 
        details: error instanceof Error ? error.message : 'Unknown error' 
      });
    }
  });
...
// Helper functions for Docker container configurations
function getContainerConfig(scenario: string, containerName: string) {
  const configs: Record<string, Record<string, any>> = {
    'docker-escape': {
      'vulnerable-app': {
        image: 'threatlab/vulnerable-web:latest',
        ports: ['8080:80'],
        environment: ['VULNERABLE=true', 'LOG_LEVEL=debug']
      },
      'privileged-container': {
        image: 'threatlab/privileged-escape:latest',
        ports: [],
        environment: ['PRIVILEGED=true'],
        privileged: true
      },
      'host-monitor': {
        image: 'threatlab/host-monitor:latest',
        ports: ['9090:9090'],
        environment: ['MONITOR_HOST=true']
      }
    },
    'lateral-movement': {
      'web-frontend': {
        image: 'nginx:alpine',
        ports: ['80:80'],
        environment: ['NGINX_HOST=frontend']
      },
      'api-backend': {
        image: 'threatlab/api-backend:latest',
        ports: ['3000:3000'],
        environment: ['API_MODE=vulnerable']
      },
      'database': {
        image: 'mysql:8.0',
        ports: ['3306:3306'],
        environment: ['MYSQL_ROOT_PASSWORD=vulnerable123']
      },
      'network-scanner': {
        image: 'threatlab/network-scanner:latest',
        ports: [],
        environment: ['SCAN_NETWORK=true']
      }
    }
  };
  
  return configs[scenario]?.[containerName] || {
    image: `threatlab/${containerName}:latest`,
    ports: [],
    environment: []
  };
}

function generateAccessInstructions(scenario: string, containers: any[]) {
  const instructions = [
    `Scenario "${scenario}" deployed successfully`,
    'Access points:',
    ...containers.map(c => 
      c.ports.length > 0 
        ? `  - ${c.name}: http://VM_IP:${c.ports[0].split(':')[0]}`
        : `  - ${c.name}: Background service`
    ),
    '',
    'Log monitoring:',
    '  - All container logs forwarded to XSIAM broker at 192.168.100.124:514',
    '  - Monitor XSIAM for threat detection alerts',
    '',
    'Next steps:',
    '  1. Generate threat activity within containers',
    '  2. Validate detection rules in XSIAM',
    '  3. Test response playbooks'
  ];
