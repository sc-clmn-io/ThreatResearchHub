...
export default function Dashboard() {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `security-research-platform-export-${new Date().toISOString().split('T')[0]}.json`;
    ...
    try {
      ...
      if (trainingPaths.length > 0) {
        trainingPaths.forEach((tp: any, index: number) => {
          const playbookYaml = {
            id: `security-research-platform-${tp.title.toLowerCase().replace(/[^\w]/g, '-')}`,
            version: 1,
      ...
      if (useCases.length > 0) {
        const dashboardData = {
          dashboards_data: [{
            ...
            global_id: `security-research-platform-dashboard-${Date.now()}`,
            metadata: { params: [] }
          }],
          widgets_data: useCases.slice(0, 5).map((uc: any, index: number) => ({
            ...
            creator_mail: "security-research-platform@security.local"
          }))
        };
      ...
      if (useCases.length > 0) {
        useCases.slice(0, 3).forEach((uc: any, index: number) => {
          const alertLayout = {
            ...
            group: "incident",
            id: `security-research-platform-layout-${uc.title.toLowerCase().replace(/[^\\w]/g, '-')}`,
            itemVersion: "1.0.0",
            kind: "",
...
`;
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `security-research-platform-export-${timestamp}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    ...
    if (confirm("Are you sure you want to clear all training data? This will refresh the page.")) {
      try {
        ...
        for (const db of databases) {
          if (db.name === 'security-research-platform-db') {
            ...
            <div className="bg-white rounded-lg shadow-sm border p-4">
              ...
              <div className="space-y-2">
                ...
                <Tooltip>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        toast({
                          ...
                          description: "CVE-YYYY-NNNN and other sample threats loaded for testing",
                        });
                        setTimeout(() => window.location.reload(), 1000);
                      }}
                    >
                  ...
                  <TooltipContent>
                    <p>Load sample threat scenarios (CVE-YYYY-NNNN, OWASP Top 10) for testing workflows</p>
                  </TooltipContent>
                </Tooltip>
