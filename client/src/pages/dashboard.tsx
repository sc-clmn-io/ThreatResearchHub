...
import { Shield, Rss, Share2, Server, Settings, Upload, Database, Layout, FileText, Layers, Lock, Archive } from "lucide-react";
import { Link } from "wouter";
import ThreatInput from "@/components/threat-input";
import UseCaseList from "@/components/use-case-list";
import TrainingPath from "@/components/training-path";
import ProgressTracker from "@/components/progress-tracker";
import ValidationQueue from "@/components/validation-queue";
import TrainingModal from "@/components/training-modal";
import LabEnvironmentSetup from "@/components/lab-environment-setup";
import XSIAMIntegrationGuide from "@/components/xsiam-integration-guide";
import AttackSimulationGuide from "@/components/attack-simulation-guide";
import AnalystWorkflowGuide from "@/components/analyst-workflow-guide";
import BulkProcessingModal from "@/components/bulk-processing-modal";
import DataSourceIntegrationGuide from "@/components/data-source-integration-guide";
import { TaxonomyBreakdownModal } from "@/components/taxonomy-breakdown";
import ProductionDeploymentGenerator from "@/components/production-deployment-generator";
...
export default function Dashboard() {
  ...
  const [taxonomyBreakdownOpen, setTaxonomyBreakdownOpen] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <Shield className="text-cortex-blue text-2xl mr-3" />
                <span className="text-xl font-medium text-cortex-dark">
                  ThreatResearchHub
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center bg-cortex-light rounded-full px-3 py-1">
                <i className="fas fa-user-circle text-cortex-blue mr-2"></i>
                <span className="text-sm font-medium">Training Engineer</span>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <ThreatInput />
            <UseCaseList onGenerateTraining={setSelectedTrainingPath} />
            <TrainingPath />
          </div>

          {/* Right Column - Progress & Validation */}
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-material p-6">
              <h2 className="text-lg font-medium text-cortex-dark mb-4">Quick Actions</h2>
              <div className="space-y-3">
                <Dialog>
                  <DialogTrigger asChild>
                    ...
                    >
                      <div className="flex items-center">
                        <Server className="h-4 w-4 text-green-500 mr-3" />
                  ...
                  <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                      <DialogTitle>Complete Lab Environment Setup</DialogTitle>
                    </DialogHeader>
                    {labSetupPhase === 'setup' && (
                      <LabEnvironmentSetup
                        useCase={labSetupUseCase || { title: 'Custom Lab', category: 'endpoint' }}
                        onComplete={(config) => {
                          setLabConfig(config);
                          setLabSetupPhase('datasource');
                        }}
                      />
                    )}
                    {labSetupPhase === 'datasource' && (
                      <DataSourceIntegrationGuide
                        useCase={labSetupUseCase || { title: 'Custom Lab', category: 'endpoint' }}
                        onIntegrationComplete={(results) => {
                          setLabConfig((prev: any) => ({ ...prev, dataSourceConfig: results }));
                ...
                >
                  <div className="flex items-center">
                    <Upload className="h-4 w-4 text-purple-500 mr-3" />
                    Bulk Processing
                  </div>
                </Button>
                ...
                <Link href="/dataset-schemas">
                  ...
                  >
                    <div className="flex items-center">
                      <Database className="h-4 w-4 text-orange-500 mr-3" />
      ...
      {selectedTrainingPath && (
        <TrainingModal
          trainingPathId={selectedTrainingPath}
          onClose={() => setSelectedTrainingPath(null)}
        />
      )}
      ...
      <BulkProcessingModal
        ...
        onComplete={(totalUseCases) => {
          toast({
            ...
            description: `Successfully extracted ${totalUseCases} use cases from multiple reports`,
          });
          setBulkProcessingOpen(false);
          // Refresh the page to show new use cases
          setTimeout(() => window.location.reload(), 1000);
        }}
      />

      {/* TRAM Taxonomy Breakdown Modal */}
      <TaxonomyBreakdownModal
        isOpen={taxonomyBreakdownOpen}
        onClose={() => setTaxonomyBreakdownOpen(false)}
      />
    </div>
  );
}
